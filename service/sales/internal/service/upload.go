...
type (
	SaleService interface {
		...
		Handle(tx *sql.Tx, pr *models.Sale, moduleID int) error
	}
...
// Handle
func (s *UploadSvc) Handle(ctx context.Context, req *treegrid.PostRequest) (*treegrid.PostResponse, error) {
...
func (s *UploadSvc) handle(tr *treegrid.MainRow) error {
	...
	if tr.Status() == 1 {
		...
		if err := s.saleSvc.Handle(tx, entity, s.moduleID); err != nil {
