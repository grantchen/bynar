...
type UploadService struct {
	db                                  *sql.DB
	updateGRLanguageRepository          treegrid.SimpleGridRowRepository
	updateGRLanguageRepositoryWithChild treegrid.GridRowRepositoryWithChild
...
func NewUploadService(db *sql.DB,
	...
	return &UploadService{
		db:                                  db,
		updateGRLanguageRepository:          updateGRLanguageRepository,
		updateGRLanguageRepositoryWithChild: updateGRLanguageRepositoryWithChild,
...
func (u *UploadService) Handle(req *treegrid.PostRequest) (*treegrid.PostResponse, error) {
	...
	trList, err := treegrid.ParseRequestUpload(req, u.updateGRLanguageRepositoryWithChild)
...
func (s *UploadService) save(tx *sql.Tx, tr *treegrid.MainRow) error {
	if err := s.saveLanguage(tx, tr); err != nil {
		userTemplateData := map[string]string{
			"Message": err.Error(),
		}
		return i18n.TranslationI18n(s.language, "SaveLanguage", userTemplateData)
	}

	if err := s.saveLanguageLine(tx, tr, tr.Fields.GetID()); err != nil {
